...
def greatestDiagonalProductL2R(grid):
    rows = len(grid)
    columns = len(grid[0])
    prod = 0
...
def greatestDiagonalProductR2L(grid):
    rows = len(grid)
    columns = len(grid[0])
    prod = 0
    for i in range(0, columns-3):
...
def greatestProductInGrid(grid):
    return max([greatestHorizontalProduct(grid),greatestVerticalProduct(grid),greatestDiagonalProductL2R(grid),greatestDiagonalProductR2L(grid)])
...
print(greatestProductInGrid(array))